...
const MAX_ZIP_BYTES = 75 * 1024 * 1024; // 75MB cap for ZIP payloads
const MAX_ENTRY_BYTES = 20 * 1024 * 1024; // limit individual CV size to 20MB
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_ZIP_BYTES) {
      return NextResponse.json({ error: "ZIP archive exceeds the 75MB limit" }, { status: 413 });
    }
